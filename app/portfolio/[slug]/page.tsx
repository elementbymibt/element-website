...
import { getLocalizedProjectText, getProjectBySlug, projects } from "@/src/data/projects";
...
export default async function ProjectDetailPage({ params }: ProjectDetailPageProps) {
  ...
  const localizedCategory = textByLocale(locale, {
    sr: project.category,
    en:
      project.category === "Stan"
        ? "Apartment"
        : project.category === "Kuća"
          ? "House"
          : "Business space",
    de:
      project.category === "Stan"
          ...
          <div>
            <h2 className="text-brand-gold text-sm tracking-[0.18em] uppercase">
              {textByLocale(locale, {
                ...
                de: "Unser Leistungsumfang",
              })}
            </h2>
            <ul className="text-brand-ink mt-3 space-y-2 text-sm">
              {localizedProject.whatWeDid.map((item) => (
